...
var genRecurringDonationInvoice = function (customer) {
    return {
        Invoice: {
            Type: 'ACCREC' ,
            Contact: {
...
var generateRecurringXML = function (customer) {
    var xmlToSend = '<?xml version="1.0" encoding="utf-8"?>\r\n';
    xmlToSend =xmlToSend + '<Request>\r\n';//<Invoices>\r\n';
    var easyxml = require('easyxml');
    easyxml.configure({ manifest: false,rootElement: 'Invoice'  }); 
 
    if (customer.donationAmount > 0) {
        var d = genRecurringDonationInvoice(customer);
        xmlToSend = xmlToSend + easyxml.render(d.Invoice).toString();
    }
    xmlToSend +='</Request>\r\n';
    console.log(xmlToSend);
...
exports.raise = function (customer, callback) {
    ...
    var xml = generateXML(customer, false);
...
exports.raiseRecurring = function (customer, callback) {
    ...
    var xml = generateRecurringXML(customer, true);
     
    xero.call('POST', '/Invoices', xml, function(err, result) {
        if (err) {
            console.log('error');
            console.error(err);
            return callback(err, xml);
